...
export default function RotatingProjects() {
  ...
  return (
    <div style={{
      ...
      <h1 style={{ fontSize: '2.5rem', marginBottom: '1rem', color: '#CFFF04' }}>{project.name}</h1>
      <span style={{ fontSize: '1.2rem', color: '#CFFF04', fontWeight: 'bold' }}>{getTypeLabel(project.type)}</span>
    </div>
  );
}
